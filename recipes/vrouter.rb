...
if node['contrail']['manage_nova_compute'] then
    ...
    if not platform?("redhat", "centos", "fedora")
        pkgs = %w( contrail-nodemgr contrail-setup contrail-vrouter contrail-vrouter-init linux-crashdump python-iniparse )
    else
        pkgs = %w( abrt contrail-nodemgr contrail-setup contrail-vrouter contrail-vrouter-init openstack-utils python-thrift contrail-vrouter-common )
    end
end
