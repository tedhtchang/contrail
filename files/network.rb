...
module ::Openstack
  ...
  def address_for(interface, family = node['openstack']['endpoints']['family'], nodeish = node, drop_vips = true)
    ...
    addr = get_address addresses, family, drop_vips
    if node['roles'].include?('contrail-icm-compute') and interface == node['contrail']['compute']['interface']
      if addr.nil?
        addresses = nodeish['network']['interfaces']['vhost0']['addresses']
        addr = get_address addresses, family, drop_vips
      end
    end
    fail "No address for family #{family} on interface #{interface} found" if addr.nil?
    addr
  end
